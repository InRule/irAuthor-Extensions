...
namespace UndoExtension
{
    public class UndoExtension : ExtensionBase
    {
        ...
        public override void Enable()
        {
            ...
            }).Subscribe(UndoStack.AsObserver());
            UndoStack.Count().Subscribe(x => undoCommand.IsEnabled = x > 0);
            UndoStack.Zip(UndoClicked, (source1, source2) => source1).Subscribe(UndoDefRemoved);
        ...
        private void UndoDefRemoved(UndoHistoryItem item)
        {
                ...
                IContainsRuleElements parent = ruleApp.LookupItem(item.ParentGuid) as IContainsRuleElements;
